...
function initializeDatabase(callback) {
    ...
    function tableCreated() {
        tablesCreated++;
        console.log(`Tables created: ${tablesCreated}/${totalTables}`);
        if (tablesCreated === totalTables) {
            console.log("All tables created. Now inserting sample data...");
...
function insertSampleData(callback) {
    let sampleDataInserted = 0;
    const totalSampleData = 2;

    function sampleDataDone() {
        sampleDataInserted++;
        console.log(`Sample data inserted: ${sampleDataInserted}/${totalSampleData}`);
        if (sampleDataInserted === totalSampleData) {
    ...
    db.get("SELECT COUNT(*) as count FROM bikes", (err, row) => {
        if (err) {
            console.error("Error checking bikes table:", err.message);
            sampleDataDone();
        } else if (row.count === 0) {
            ...
            sampleBikes.forEach(bike => {
                stmt.run(bike.name, bike.type, bike.price, bike.description, bike.image, (err) => {
                    if (err) {
                        console.error("Error inserting bike:", err.message);
                    }
                    bikesInserted++;
                    if (bikesInserted === sampleBikes.length) {
                        stmt.finalize(() => {
            ...
            console.log("Sample bikes already exist");
            sampleDataDone();
        }
    });

    // Insert admin user
    db.get("SELECT COUNT(*) as count FROM users WHERE email = 'admin@bikerentpro.com'", (err, row) => {
        if (err) {
...
app.put('/api/profile', authenticateToken, (req, res) => {
    ...
    db.get("SELECT * FROM users WHERE id = ?", [userId], (err, user) => {
        ...
        // Update user profile
        db.run(
            "UPDATE users SET name = ?, email = ?, phone = ?, date_of_birth = ?, address = ? WHERE id = ?",
            [name, email, phone, date_of_birth, address, userId],
            function(err) {
                if (err) {
                ...
                db.get("SELECT id, name, email, phone, avatar, date_of_birth, address, member_since, is_verified FROM users WHERE id = ?", [userId], (err, updatedUser) => {
                    if (err) {
                        return res.status(500).json({ message: 'Database error' });
                    }
